...
modelSS = createpde("thermal","steadystate-axisymmetric");
...
thermalProperties(modelSS,'Face',1, "ThermalConductivity",k_m);
thermalProperties(modelSS,'Face',2, "ThermalConductivity",k_s);
thermalProperties(modelSS,'Face',3, "ThermalConductivity",k_t);

internalHeatSource(modelSS, @(location,state) rho_b*cp_b*w_m*(T_b- state.u), 'Face',1);
internalHeatSource(modelSS, @(location,state) rho_b*cp_b*w_s*(T_b- state.u), 'Face',2);
internalHeatSource(modelSS, @(location,state) rho_b*cp_b*w_t*(T_b- state.u), 'Face',3);

internalHeatSource(modelSS, Q_m, 'Face',1);
internalHeatSource(modelSS, Q_s, 'Face',2);
internalHeatSource(modelSS, Q_t, 'Face',3);
...
figure 
pdeplot(modelSS,"XYData",SST.Temperature,"Contour","on")
axis equal
...
thermalProperties(modelTS,'Face',1, "ThermalConductivity",k_m,"MassDensity", rho_m,"SpecificHeat",cp_m);
thermalProperties(modelTS,'Face',2, "ThermalConductivity",k_s,"MassDensity", rho_s,"SpecificHeat",cp_s);
thermalProperties(modelTS,'Face',3, "ThermalConductivity",k_t,"MassDensity", rho_t,"SpecificHeat",cp_t);

internalHeatSource(modelTS, @(location,state) rho_b*cp_b*w_m*(T_b- state.u), 'Face',1);
internalHeatSource(modelTS, @(location,state) rho_b*cp_b*w_s*(T_b- state.u), 'Face',2);
internalHeatSource(modelTS, @(location,state) rho_b*cp_b*w_t*(T_b- state.u), 'Face',3);

internalHeatSource(modelTS, Q_m, 'Face',1);
internalHeatSource(modelTS, Q_s, 'Face',2);
internalHeatSource(modelTS, Q_t, 'Face',3);

internalHeatSource(modelTS, Q_MNP, 'Face',3);

thermalBC(modelTS,"Edge",[9,10],"ConvectionCoefficient",htc,"AmbientTemperature",T_amb);

thermalIC(modelTS, T_b);
