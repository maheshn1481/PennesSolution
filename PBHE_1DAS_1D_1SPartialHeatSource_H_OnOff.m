...
for n = 2:TS
    Force = zeros(N,1);
    Force(1:N-1)  = T(1:N-1,n-1) + rho_b*cp_b.*w_t*dt*T_b/(rho_t*cp_t) + Qm_t*dt/(rho_t*cp_t) + q_mnp(1:N-1,n)*dt/(rho_t*cp_t);
    Force(N) = T_b;
    T(:,n) = thomas_algorithm(Lower, Main, Upper, Force);
end
% T2 = zeros(N, TS);
% T2(:,1) = T_b;
% for n = 2:TS
%     Force = zeros(N,1);
%     Force(1:N-1)  = T2(1:N-1,n-1) + rho_b*cp_b.*w_t*dt*T_b/(rho_t*cp_t) + Qm_t*dt/(rho_t*cp_t) + q_mnp(1:N-1,n)*dt/(rho_t*cp_t);
%     Force(N) = T_b;
%     T2(:,n) = tridiag(Lower, Main, Upper, Force);
% end
...
% figure()
% subplot(1,2,1)
% plot(t,T2(1,:),t,T2(find(RT==r),:), 'LineWidth',2)
% xlabel('Time, t [s]');
% ylabel('Temperature, T [°C]');
% xlim([0,t_end])
% xticks(0:200:t_end);
